...
class GPT(nn.Module):
    ...
    def extend_block_size(self, block_size):
        assert block_size >= self.config.block_size
        self.config.block_size = block_size

        # Extend the position embedding matrix
        old_wpe = self.transformer.wpe.weight
        new_wpe = torch.empty(block_size, old_wpe.size(1))
        torch.nn.init.xavier_uniform_(new_wpe)
        new_wpe[:old_wpe.size(0)] = old_wpe
        self.transformer.wpe.weight = nn.Parameter(new_wpe)

        # Extend the attention bias
        for block in self.transformer.h:
            old_bias = block.attn.bias
            new_bias = torch.empty(*old_bias.size()[:-2], block_size, block_size)
            torch.nn.init.xavier_uniform_(new_bias)
            new_bias[:,:,:old_bias.size(-2),:old_bias.size(-1)] = old_bias
    ...
    def from_pretrained(cls, model_type, override_args=None):
      #  assert model_type in {'gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl'}
        ...
        config_args = {
            '111m':         dict(n_layer=10, n_head=12, n_embd=768),  # 124M params
            '256':         dict(n_layer=14, n_head=17, n_embd=1088),  # 124M params
        ...
        config_args['block_size'] = 2048 # always 1024 for GPT model checkpoints
        config_args['bias'] = True # always True for GPT model checkpoints
        print(f"forcing vocab_size={config_args['vocab_size']}, block_size={config_args['block_size']}, bias=True")
    ...
    @torch.no_grad()
    def generate_instructional(self, idx, idi, max_new_tokens, temperature=1.0, top_k=None):
        """
        Take a conditioning sequence of indices idx (LongTensor of shape (b,t)) and complete
        the sequence max_new_tokens times, feeding the predictions back into the model each time.
        Most likely you'll want to make sure to be in model.eval() mode of operation for this.
